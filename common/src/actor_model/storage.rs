...
// ArenaMap
impl<K: ArenaKey, V> FromIterator<(K, V)> for ArenaMap<K, V> {
    fn from_iter<T: IntoIterator<Item = (K, V)>>(iter: T) -> Self {
        let mut ret: ArenaMap<K, V> = Self::default();
        for (k, v) in iter {
            ret.insert(k, v);
        }
        ret
...
impl<K: ArenaKey, V> Map<K, V> for ArenaMap<K, V> {
    type Iter<'a> = impl Iterator<Item = (K, &'a V)> where K: 'a, V: 'a;
    type IterMut<'a> = impl Iterator<Item = (K, &'a mut V)> where K: 'a, V: 'a;
    ...
    fn get_mut(&mut self, k: K) -> Option<&mut V> {
        ArenaMap::get_mut(self, k)
    }

    fn insert(&mut self, k: K, v: V) -> Option<V> {
        ArenaMap::insert(self, k, v)
    }

    fn iter(&self) -> Self::Iter<'_> {
        Box::new(ArenaMap::iter(self))
    }

    fn iter_mut(&mut self) -> Self::IterMut<'_> {
        Box::new(ArenaMap::iter_mut(self))
    }
    ...
    where
        V: Default,
    {
        ArenaMap::entry(self, k).or_insert_with(Default::default)
    }

    fn remove(&mut self, k: K) -> Option<V> {
        ArenaMap::remove(self, k)
    }

    fn retain(&mut self, f: impl FnMut(K, &mut V) -> bool) {
        ArenaMap::retain(self, f)
    }
}

impl<K, V> OrdIter for ArenaMap<K, V> {}
impl<K, V> Efficient for ArenaMap<K, V> {}
