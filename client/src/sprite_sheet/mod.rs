...
pub struct SpriteSheet {
    ...
    pub sprites: HashMap<String, SpriteGeo>,
    /// Each animation has a name and multiple sprites to cycle through.
    #[serde(serialize_with = "ordered_map")]
    pub animations: HashMap<String, Vec<SpriteGeo>>,
}

/// Sprite stores pixel coordinates.
#[derive(Serialize, Deserialize)]
pub struct SpriteGeo {
    pub x: u32,
    pub y: u32,
    pub width: u32,
    pub height: u32,
...
impl SpriteGeo {
    /// Position of [`SpriteGeo`] equivilant to `uvec2(x, y)`.
    pub fn position(&self) -> UVec2 {
        uvec2(self.x, self.y)
    }

    /// Dimensions of [`SpriteGeo`] equivilant to `uvec2(width, height)`.
    pub fn dimensions(&self) -> UVec2 {
        uvec2(self.width, self.height)
    }

    /// Converts a [`SpriteGeo`] into a [`UvSprite`]. Requires the dimensions of the [`SpriteSheet`].
