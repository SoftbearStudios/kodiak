...
/// [`derive_vertex`][`super::derive_vertex`].
...
/// your `Cargo.toml`.  This macro assumes that `renderer` is defined.
pub macro derive_vertex {
    ($s:item) => {
        #[derive(Copy, Clone, $crate::renderer::Vertex, $crate::renderer::__Pod, $crate::renderer::__Zeroable)]
        #[repr(C)]
        $s
    }
}
