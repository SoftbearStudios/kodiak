...
use super::renderer::{Layer, Renderer};
use super::vertex::Vertex;
use super::{DefaultRender, RenderLayer, ShaderBinding};
use kodiak_common::fxhash::FxHashMap;
...
impl<V: Vertex, I: Index, M: Vertex, ID: MeshId> InstanceLayer<V, I, M, ID> {
    ...
    /// Returns true if no instances were drawn. Good for early exit.
    pub fn is_empty(&self) -> bool {
        self.buffers.iter().all(|(_, b)| {
            b.instances.is_empty()
                && match &b.inner {
                    InnerBuffers::Render(_, instance_buffer) => instance_buffer.is_empty(),
                    _ => true,
                }
        })
    }
