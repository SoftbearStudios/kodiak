...
use super::{Antialiasing, OwnedFramebufferBinding};
use crate::js_hooks::{self, error_message};
pub use crate::Layer;
use kodiak_common::glam::{uvec2, UVec2, Vec4};
...
use super::srgb_layer::SrgbLayer;
...
impl<L: Layer> RenderChain<L> {
    ...
    pub fn new(
        ...
        antialiasing: impl Into<Antialiasing>,
...
pub struct Renderer {
    ...
    /// To allow binding [`Framebuffer`][`super::Framebuffer`]s recursively.
...
impl Renderer {
    ...
    pub(crate) fn new(
        ...
        let builtin_aa = builtin_aa && antialiasing.is_msaa();
        let _ = alpha; // TODO
        ...
        #[cfg(all(not(feature = "renderer_webgl2"), feature = "renderer_depth_texture"))]
...
impl Renderer {
    ...
    /// Call early on if using [`prim@u32`] as [`Index`][`super::index::Index`].
    ...
    #[deprecated = "use include_shader!(renderer, \"name\") instead"]
    pub fn create_shader(&self, vertex: &'static str, fragment: &'static str) -> Shader {
        self.include_shader(vertex, fragment)
    }

    /// Don't call this directly. Use `include_shader` instead.
    #[doc(hidden)]
    pub fn include_shader(&self, vertex: &'static str, fragment: &'static str) -> Shader {
        self.shader_cache
            .borrow_mut()
    ...
    pub fn invert_depth(&self, invert: bool) {
        self.gl
            .depth_func(if invert { Gl::GREATER } else { Gl::LESS });
    }

    // TODO doc
    #[doc(hidden)]
    pub fn invert_depth_equal(&self, invert: bool) {
        self.gl
            .depth_func(if invert { Gl::GEQUAL } else { Gl::LEQUAL });
    }
    ...
    /// Helper for shadow volumes in renderer3d.
    #[doc(hidden)]
    pub fn shadow_volume_stencil(&self, stage: u8) {
        match stage {
            0 => {
                self.gl.enable(Gl::STENCIL_TEST);
                // Default stencil_func is (Gl::ALWAYS, 0, u32::MAX).
                self.gl
                    .stencil_op_separate(Gl::FRONT, Gl::KEEP, Gl::KEEP, Gl::INCR_WRAP);
                self.gl
                    .stencil_op_separate(Gl::BACK, Gl::KEEP, Gl::KEEP, Gl::DECR_WRAP);
                self.gl.disable(Gl::CULL_FACE);
            }
            1 => {
                self.gl.enable(Gl::CULL_FACE);
                self.gl
                    .stencil_op_separate(Gl::FRONT_AND_BACK, Gl::KEEP, Gl::KEEP, Gl::KEEP);
                self.gl.stencil_func(Gl::NOTEQUAL, 0, u32::MAX);
            }
            2 => {
                self.gl.stencil_func(Gl::ALWAYS, 0, u32::MAX);
    ...
    pub fn set_scissor(&self, x: u32, y: u32, dimensions: UVec2) {
        self.gl.enable(Gl::SCISSOR_TEST);
        self.gl
            .scissor(x as i32, y as i32, dimensions.x as i32, dimensions.y as i32);
    }
