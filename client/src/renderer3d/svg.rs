...
use super::{Camera3d, Orthographic};
use kodiak_common::glam::{IVec3, Vec2, Vec3};
...
impl Model {
    fn triangles(&self) -> impl Iterator<Item = Triangle> + '_ {
        let vertex_floats = self.vertex_len();
        self.indices
            .as_chunks()
            .0
            .iter()
            .map(move |indices: &[u16; 3]| {
                let points = indices.map(|idx| {
                    Vec3::from_slice(bytemuck::cast_slice(
                        &self.vertices[idx as usize * vertex_floats..],
                    ))
                });
    ...
    pub fn svg(&self) -> String {
        ...
        for triangle in &triangles {
            ...
            write!(
                ...
                crate::renderer::rgba_array_to_css([(light * u8::MAX as f32) as u8, 0, 0, 255])
            )
            .unwrap();
        ...
        use base64::prelude::*;
        svg.push_str("</svg>");
        let encoded = format!("data:image/svg+xml;base64,{}", BASE64_STANDARD.encode(svg));
        encoded
    }
}
