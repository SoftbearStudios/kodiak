...
use crate::renderer::{
    DefaultRender, Layer, PointDeque, RenderLayer, Renderer, ShaderBinding, Vertex,
};
...
impl<X: Particle> ParticleLayer<X> {
    ...
    /// Adds a particle part of it's lifespan used up by `age`. Useful for fps independent particles.
    /// TODO replace `add` with this since all particles should be fps independent.
    pub fn add_aged(&mut self, p: X, age: f32) {
        self.buffer.push_back(ParticleVertex {
            inner: p,
            created: self.time + age,
        });
    }
