...
use crate::renderer::{
    derive_vertex, include_shader, DefaultRender, Index, Layer, MeshBuilder, RenderLayer, Renderer,
    Shader, TriangleBuffer,
};
use kodiak_common::glam::{Mat2, Vec2, Vec4};
...
impl<I: Index> DefaultRender for GraphicLayer<I> {
    fn new(renderer: &Renderer) -> Self {
        Self {
            shader: include_shader!(renderer, "graphic"),
...
impl<I: Index> GraphicLayer<I> {
    ...
    /// rotated by `angle`. An `angle` of 0 is pointing up.
    ...
    /// Draws a filled convex polygon defined by `vertices` (pos, color).
    pub fn draw_convex_polygon(&mut self, vertices: impl Iterator<Item = (Vec2, Vec4)>) {
        let start = self.mesh.vertices.len();
        self.mesh
            .vertices
            .extend(vertices.map(|(pos, color)| PosColor { pos, color }));

        let len = self.mesh.vertices.len() - start;
        if len < 3 {
            return; // No triangles.
        }
        ...
        for b in 1..len - 1 {
            let c = b + 1;
            self.mesh.indices.extend_from_slice(&[
                I::from_usize(start + a),
                I::from_usize(start + b),
                I::from_usize(start + c),
            ]);
        }
    }
}
