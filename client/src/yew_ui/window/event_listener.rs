...
/// Listens to a certain event type on the window/document/body.
pub struct GlobalEventListener<E> {
    _inner: EventListener,
    _spooky: PhantomData<E>,
}

impl<E: JsCast> GlobalEventListener<E> {
    fn new_inner(
        target: &EventTarget,
        name: &'static str,
    ...
    /// Goes on the document instead of the window.
    pub fn new_document(
        name: &'static str,
        callback: impl FnMut(&E) + 'static,
        allow_prevent_default: bool,
