...
use crate::{use_set_context_menu_callback, GlobalEventListener};
use gloo::timers::callback::Timeout;
use stylist::yew::styled_component;
use web_sys::MouseEvent;
use yew::html::IntoPropValue;
use yew::{
    function_component, hook, html, use_effect_with, use_state, Callback, Children, Html,
    Properties,
};
...
pub fn context_menu(props: &ContextMenuProps) -> Html {
    ...
    use_effect_with((props.position.clone(), *activity), |_| {
        let listener = GlobalEventListener::new_body(
            "contextmenu",
            move |e: &MouseEvent| {
                e.prevent_default();
                e.stop_propagation();
